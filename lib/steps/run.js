...
function openInstance () {
  if (OS === 'linux') {
    try {
      ...
      try {
        ...
        console.log("Please open instance manually!")
      }
    }
  } else {
    try {
  ...
  console.log('Everything installed'.green)
  console.log('Store URL: https://scandipwa.local\nAdmin URL: https://scandipwa.local/admin\nAdmin Login: admin\nAdmin Pass: scandipwa123'.cyan)
  process.exit()
}

module.exports = async (mode) => {
  let instanceReady = false
  console.log('\nPulling docker images'.yellow)
  await execSync('cd scandipwa-base && docker-compose -f docker-compose.yml -f docker-compose.local.yml -f docker-compose.ssl.yml -f docker-compose.frontend.yml pull', { stdio: 'inherit' })
  console.log('Pulled docker images'.green)
  ...
  if (instanceReady) {
    ...
    if (!runned) {
      ...
      await execAsyncStream(`docker exec -u user ${container} composer require scandipwa/sample-data`,
        { cwd: 'scandipwa-base' },
        console.log).catch(async () => await console.log('Demo is not imported! Import it manually!'))
      await execAsyncStream(`docker exec -u user ${container} php bin/magento setup:upgrade`,
        { cwd: 'scandipwa-base' },
        console.log).catch(async () => await console.log('Setup upgrade failed, please do it manually!'))
    }
