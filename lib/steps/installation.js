  ...
  try {
    ...
    try {
      ...
      try {
        ...
        try {
          await execAsync('curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add –')
          await execAsync('sudo add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/ubuntu  $(lsb_release -cs)  stable"')
          await execAsync('sudo apt-get install docker-ce')
        } catch (e) {
